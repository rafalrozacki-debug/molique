...
.accordion-item {
  ...
  &::details-content {
    ...
    transition: height var(--transition-speed-slow) cubic-bezier(0.4, 0, 0.2, 1), content-visibility var(--transition-speed-slow) allow-discrete;
    height: 0;
    overflow: hidden;
  }
  ...
  &[open]::details-content {
    ...
    overflow: visible;
    transition:
      height var(--transition-speed-slow) cubic-bezier(0.4, 0, 0.2, 1),
      content-visibility var(--transition-speed-slow) allow-discrete,
      overflow 0s var(--transition-speed-slow) allow-discrete;
  }

  /* A11y: bez animacji wysokosci dla uzytkownikow z prefers-reduced-motion -
     akordeon otwiera/zamyka sie od razu, bez plynnego rozwijania. */
...
@layer components {
  ...
  .admin-nav-dropdown-menu {
    ...
    @include mq(md, max) {
      ...
      transition: transform var(--transition-speed-slow) cubic-bezier(0.2, 0.8, 0.2, 1), visibility var(--transition-speed-slow);
...
@layer components {
  .admin-sidebar {
    ...
    /* FIX: Sidebar ma sztywną szerokość i płynnie rośnie.
       Świadome odstępstwo od reguły GPU-only (transform/opacity):
       animowanie width wywołuje reflow, ale to jednorazowa, inicjowana
       kliknięciem zmiana (przełącznik szerokości), nie ciągła/zapętlona
       animacja - koszt jest jednorazowy, nie per-klatkowy. Ten sam
       kompromis co .progress-bar/.progress-bar-reading w _progress.scss. */
  ...
  .sidebar-toggle-icon {
    ...
    position: relative;
    display: inline-block;
    transition: transform var(--transition-speed-slower) cubic-bezier(0.4, 0, 0.2, 1);
    ...
    span {
      ...
      background-color: currentColor;
      border-radius: 2px;
      transition: all var(--transition-speed-slower) cubic-bezier(0.4, 0, 0.2, 1);
      width: 100%;
    }
...
.featured-box {
  ...
  /* Cien hover na ::after (animowana tylko opacity) zamiast bezposredniego
     przejscia box-shadow (reflow) - ten sam wzorzec co .hover-gpu-shadow. */
  &::after {
    content: '';
    position: absolute;
    inset: 0;
...
.thumb-info-wrapper {
  position: absolute;
  inset: 0;
  background-color: rgba(var(--backdrop-rgb), 0.6);
  opacity: 0;
...
.carousel-nav {
  ...
  /* Wizualny rozmiar (40px) zostaje - niewidoczny obszar klikalny dobity
     do --target-size-min (44px). */
  &::before {
    content: '';
    position: absolute;
    top: 50%;
...
.carousel-dot {
  ...
  /* Zwiększenie obszaru klikalnego do --target-size-min (44px) */
  &::before {
    content: '';
    position: absolute;
    top: 50%;
...
.sparkline-bar {
  ...
  transition: height var(--transition-speed-chart) cubic-bezier(0.2, 0.8, 0.2, 1);
  ...
  /* A11y: bez animacji "rosniecia" dla uzytkownikow z prefers-reduced-motion -
     slupki pojawiaja sie od razu w docelowej wysokosci. */
  @media (prefers-reduced-motion: reduce) {
    transition: none;
  }
}
...
.chart-radial {
  ...
  /* Hak dla JS (molique-counters.js): gdy .chart-radial towarzyszy
     .counter-value, skrypt resetuje --val do 0% i odtwarza wypelnienie
     w momencie wejscia w widok (bez tego pierscien wypelnia sie od razu
     przy wczytaniu strony przez @starting-style nizej - niewidoczne,
     jesli sekcja jest poza pierwszym ekranem). Reset musi byc SKOKOWY,
     nie animowany, stad chwilowe wylaczenie transition na ::before. */
  &.js-resetting::before {
  ...
  /* Pierścień (conic-gradient + maska wycinająca środek) żyje na ::before,
     NIE bezpośrednio na .chart-radial: mask-image ucina render całego
     elementu WRAZ Z DZIEĆMI, więc na głównym boksie ucinałby też
     .radial-value w środku - liczba byłaby zawsze niewidoczna. */
  &::before {
    content: '';
    position: absolute;
    inset: 0;
...
/* =========================================
   NAWIGACJA WYKRESU (segmentowany przelacznik zakresu)
   =========================================
   Komponent byl od pierwszego commita opisany w slownikach i uzyty w markupie
   trzech stron, ale regula CSS nigdy nie powstala. Markup czekal gotowy:
   .chart-nav > .chart-nav-item (label) > input[type=radio] + .nav-label

   Stan trzyma radio, wiec przelaczanie dziala bez JS. Radio jest UKRYTE
   TECHNIKA CLIP, a nie display:none - to istotna roznica, bo display:none
   wyrzuca pole z kolejnosci tabulacji i z drzewa dostepnosci. Przy tej
   technice grupa radio zostaje obslugiwana strzalkami z klawiatury. */
.chart-nav {
  display: inline-flex;
  gap: 2px;
  padding: 4px;
...
.chart-nav-item {
  ...
  /* Ukrycie zachowujace fokusowalnosc (ta sama technika co .sr-only). */
  input {
    position: absolute;
    width: 1px;
    height: 1px;
  ...
  &:hover .nav-label { color: var(--text-main); }

  /* Zaznaczony zakres. Radio jest bezposrednim rodzenstwem etykiety. */
  ...
  /* Obwodka focusa musi trafic na ETYKIETE, bo samo radio jest niewidoczne. */
...
/* =========================================
  ...
  @media (max-width: 768px) {
    .popover-context::backdrop {
      background: rgba(var(--backdrop-rgb), 0.5);
...
```scss
/**
 * molique - Stopka strony (partials/footer.html)
 * Stopka jest ZAWSZE ciemna (.bg-dark) niezaleznie od motywu, jak sidebar
 * admina - dlatego linki i tlo licza sie tu wzgledem literalnych kolorow
 * .bg-dark (#14162B / #F9F9F9), a nie zmiennych --dark, ktore w dark mode
 * sie odwracaja.
 *
 * Asymetryczny odstep marka -> kolumny linkow: grid ma jeden wspolny
 * column-gap, wiec kolumna marki dostaje dodatkowo max-width + margin-right
 * (tylko od breakpointu lg) - jej TOR (1fr) zostaje nietkniety, tylko
 * widoczna zawartosc jest ograniczona i domknieta wiekszym odstepem niz
 * miedzy kolumnami 2-5.
 */
...
@layer components {
  /* Lokalne zmienne RGB dla stopki - NIE --light-rgb/--dark-rgb globalne,
     bo te odwracaja sie w dark mode (patrz komentarz na gorze pliku).
     Kazdy .footer-* nizej zachowuje WLASNA, dobrana dla siebie
     przezroczystosc - zmieniaja sie tu tylko zrodla koloru. */
  footer.bg-dark {
    --footer-muted-rgb: 249, 249, 249;
    --footer-white-rgb: 255, 255, 255;
    /* Odcien primary z dark mode (#8B84FF), nie var(--primary) light-mode'owy
       (#5145E5) - ten drugi na literalnie ciemnym tle stopki daje tylko
       2.82:1 (WCAG wymaga 4.5:1 dla tekstu). */
    --footer-primary: #8B84FF;
    --footer-primary-hover: #A5A0FF;
  }
  ...
  .footer-trustbar {
    ...
    flex-wrap: wrap;
    gap: calc(var(--spacing-unit) * 2);
    padding-bottom: calc(var(--spacing-unit) * 2.75);
    margin-bottom: calc(var(--spacing-unit) * 3.75);
    border-bottom: 1px solid rgba(var(--footer-white-rgb), 0.1);
  }

  .footer-trustbar-text {
    display: flex;
    align-items: center;
    flex-wrap: wrap;
    gap: calc(var(--spacing-unit) * 1.25);
    font-size: var(--text-sm);
    color: rgba(var(--footer-muted-rgb), 0.68);
    ...
    .footer-trustbar-dot {
      ...
      height: 3px;
      border-radius: 50%;
      background: rgba(var(--footer-white-rgb), 0.25);
    }
  }
  ...
  .footer-cols {
    display: grid;
    grid-template-columns: 1fr;
    row-gap: calc(var(--spacing-unit) * 3.25);
    padding-bottom: calc(var(--spacing-unit) * 4.5);
    align-items: start;

    @include mq(md, min) {
      grid-template-columns: 1fr 1fr;
      column-gap: calc(var(--spacing-unit) * 2.5);
    }

    @include mq(lg, min) {
      grid-template-columns: 1fr 0.6fr 0.65fr 0.65fr 0.6fr;
      column-gap: calc(var(--spacing-unit) * 2.5);
    }

    /* kolumna marki na md (2 kolumny) zajmuje caly gorny rzad */
    @include mq(md, min) {
  ...
  .footer-author {
    ...
    padding-top: calc(var(--spacing-unit) * 2.25);
    border-top: 1px solid rgba(var(--footer-white-rgb), 0.1);
  }
  ...
  .footer-author-role {
    ...
    color: rgba(var(--footer-muted-rgb), 0.6);
  }
  ...
  .footer-bottom {
    ...
    font-size: var(--text-xs);
    color: rgba(var(--footer-muted-rgb), 0.55);
  }
}
...
@layer components {
  ...
  .file-upload-animated {
    ...
    .file-upload-icon, h4, p, .file-upload-name {
      position: relative;
      z-index: 2;
      /* Bez tego te elementy (ten sam z-index co input, ale PO nim
         w DOM - wygrywaja stacking) przechwytywaly klik i drop dokladnie
         tam, gdzie wzrokowo celuje uzytkownik: na ikonie/tekscie na
         srodku karty. Klik "dzialal" tylko przypadkiem, gdy trafil w
         padding wokol nich - drop na sam srodek trafial zawsze w blok
         tekstu, nigdy w input pod spodem. */
      pointer-events: none;
    }
  }
...
@layer components {
  ...
  /* .card-body ma wlasny flex gap (spacing miedzy dziecmi) - wlasny
     margin-bottom .form-floating wyzej dublowalby ten odstep (gap +
     margines = podwojna przerwa). Margines wyzej zostaje bez zmian dla
     pozostalych kontekstow (np. kilka .form-floating wprost w <form> bez
     .d-flex) - tam to on jedyny odpowiada za odstep miedzy polami. */
...
@layer components {
  ...
  .input-range {
    ...
    border-radius: 50rem; /* pigulka - ten sam idiom co .tabs-pill */
    outline: none;
    cursor: pointer;
    overflow: hidden; 
    ...
    &::-moz-range-progress { background: var(--primary); height: 8px; border-radius: 50rem; }
    &::-moz-range-thumb {
...
.grid-expand {
  ...
  // Trzeci wyzwalacz: .form-switch tuz PRZED .grid-expand jako rodzenstwo,
  // nie zagniezdzenie - dzieki temu klik gdziekolwiek w odslonietej tresci
  // nie przelacza switcha, co nastapiloby, gdyby siedziala w tym samym
  // <label>.
  &.is-open,
  details[open] &,
  .form-switch:has(.form-switch-input:checked) + & {
...
@layer components {
  ...
  .overlay-dark { background-color: #14162B; }
...
## Plik: `components/_icons.scss`

```scss
/**
 * molique - Ikony (Phosphor, sprite SVG selektywny)
 * Zero fontu ikon, zero <img> per ikona: jeden plik img/icons-sprite.svg
 * (generowany przez tools/gen-icon-sprite.js z folderu ikon Phosphor,
 * TYLKO te faktycznie uzyte w src/*.html), referowany przez <use>.
 *
 * Uzycie:
 *   <svg class="icon"><use href="img/icons-sprite.svg#ph-lightning"></use></svg>
 *
 * Kolor: kazda sciezka ikony ma juz stroke="currentColor" (Phosphor SVG),
 * wiec .icon dziedziczy kolor tekstu automatycznie - zero CSS potrzebnego
 * per-instancja, dziala tak samo w dark mode.
 */
...
@layer components {
  .icon {
    ...
    /* Fallback dla wag "fill"/"duotone" (uzywaja fill, nie stroke) -
       stroke="currentColor" jest juz w SVG per-sciezka, ale fill tutaj
       zabezpiecza gdyby ktos uzyl symbolu bez wlasnego fill. */
    fill: currentColor;
  }

  /* Warianty rozmiaru - font-size kontroluje 1em, wiec te klasy dzialaja
     wszedzie, takze na elementach, ktore same nie ustawiaja font-size. */
...
## Plik: `components/_lang-suggest.scss`

```scss
/**
 * molique - Lang Suggest (pasek sugestii zmiany języka)
 * Markup generowany przez js/modules/molique-lang-suggest.js - wstawiany
 * na końcu <body>, tylko gdy wykryty język przeglądarki różni się od
 * języka strony (document.documentElement.lang) i istnieje jej wersja w
 * tym języku (patrz .language-switch-menu[data-lang] w navbarze).
 */
...
@layer components {
  ...
  @media (prefers-reduced-motion: reduce) {
    .lang-suggest-bar { transition: none; }
  }
...
@layer components {
  ...
  .language-switch-flag {
    ...
    svg {
      ...
      height: 100%;
      object-fit: cover;
      /* Bez tego flagę dało się przeciągnąć/zaznaczyć kursorem myszy. */
      pointer-events: none;
      user-select: none;
    }
...
// Modul niezalezny: mozna go pominac w bundlu bez bledow kompilacji.
//
// Natywny <dialog> zamiast recznie sterowanego <div> - showModal()/close()
// daja za darmo: pulapke fokusa (Tab nie wychodzi na strone pod spodem, co
// stara wersja na div NIE robila), przywrocenie fokusa elementowi
// wyzwalajacemu po zamknieciu, zamykanie klawiszem Escape i umieszczenie w
// top layer (zero wojen o z-index). ::backdrop + @starting-style to ten sam
// wzorzec, ktory _context-menu.scss juz stosuje dla swojego bottom-sheetu.
...
@layer components {
/* =========================================
   3. LIGHTBOX (Galeria pełnoekranowa, natywny <dialog>)
   ========================================= */
dialog.lightbox-overlay {
  /* Reset UA-stylesheetu <dialog> (ramka, tlo, wysrodkowanie przez margin:auto,
     ograniczenia max-width/height) - reszta to normalny flex-overlay. */
  border: none;
  padding: 0;
  background: transparent;
  color: inherit;
  ...
  /* WAZNE: display:none tutaj musi zostac bezwarunkowy. Reguly autora
     zawsze biją reguly user-agenta niezaleznie od specyficznosci selektora
     - gdyby "display: flex" bylo tutaj (bez :modal), wygralaby ZAWSZE z
     domyslnym "dialog:not([open]) { display:none }" przegladarki, nawet
     gdy dialog jest zamkniety. Dlatego flex trafia wylacznie do &:modal. */
  display: none;
  opacity: 0;
  transition: opacity var(--transition-speed), overlay var(--transition-speed) allow-discrete,
  ...
  &::backdrop {
    background: rgba(var(--backdrop-rgb), 0);
    transition: background-color var(--transition-speed) allow-discrete;
  }

  &:modal::backdrop {
    background: rgba(var(--backdrop-rgb), 0.95);
  }
...
.mega-menu-featured {
  ...
  background-image: linear-gradient(145deg, #14162B, #5145E5);
  color: #fff;
}
...
.navbar-pill {
  /* --- API kolorów: osobna zmienna na każdy stan -------------------------
     <nav class="navbar navbar-pill" style="
            ...
            --navbar-pill-bg-scrolled: #14162B;
...
/* WARIANT: Nowoczesna (Oddzielone kafelki) */
  .pagination-modern {
    gap: var(--spacing-unit); /* Odstęp między przyciskami */
    ...
    .page-link {
      ...
      /* Wizualny rozmiar (36px) zostaje - niewidoczny obszar klikalny
         dobity do --target-size-min (44px) przez ::before, bez zmiany
         wygladu. */
      &::before {
        content: '';
        position: absolute;
...
.pricing-table {
  ...
  padding: calc(var(--spacing-unit) * 4);
  text-align: center;
  /* Tylko transform w transition - box-shadow na hover zmienia sie od razu
     (jedno przemalowanie), nie w petli klatek jak przy animowanym
     przejsciu. Wzorca ::after z .featured-box nie da sie tu uzyc: ten
     kontener ma overflow:hidden (potrzebne pod wstazke .is-featured
     ponizej), wiec cien pseudo-elementu zostalby przyciety. */
  transition: transform var(--transition-speed);
  position: relative;
  overflow: hidden;
  display: flex;
...
/* =========================================
   ...
   ========================================= */
.status-dot {
  position: relative; /* punkt odniesienia dla pierscienia .status-ping::after */
  display: inline-flex;
  align-items: center;
  gap: calc(var(--spacing-unit) * 1);
  font-size: 0.875rem;
...
/* Animowany Ping - pierscien na ::after (osobny od kropki ::before),
   animowany wylacznie transform: scale + opacity zamiast box-shadow.
   Pozycjonowany wzgledem .status-dot (position: relative wyzej), dokladnie
   nad 8x8px kropka z ::before - lewy brzeg + pionowe wysrodkowanie. */
.status-ping::after {
  content: '';
  position: absolute;
  left: 0;
...
.status-checkbox {
  ...
  /* .status-icon-toggle w środku ma zajęte oba pseudo-elementy pod grafikę
     plus/checkmark, więc niewidoczny hit-area do --target-size-min (44px)
     dokładamy tutaj, na klikalnym <label> - wizualny rozmiar ikony (32px)
     zostaje bez zmian. Dotyczy tylko wariantu z tym wrapperem; goły
     <button class="status-icon-toggle"> bez <label> nie ma wolnego
     pseudo-elementu na ten sam trik (udokumentowane ograniczenie). */
  &::before {
    content: '';
    position: absolute;
    top: 50%;
...
@layer components {
  ...
  @mixin make-table-cards {
    ...
    /* Etykieta NAD wartoscia (nie obok, flexem) - komorki z bogata trescia
       (kilka <code> na raz, dluzszy opis) psuly sie w wariancie flex:
       kazdy element potomny (kazdy <code>, kazdy fragment tekstu) staje sie
       WLASNYM elementem flex i przy braku miejsca zamiast zawijac sie jako
       jeden akapit, kazdy z nich osobno kurczy sie do prawie zerowej
       szerokosci - tekst lamie sie na pojedyncze litery. Zwykly block-flow
       (bez flex/grid na komorce) jest odporny na to niezaleznie od tego,
       ile elementow inline zawiera wartosc. */
    td {
      text-align: left;
      border-bottom: 1px solid var(--border-color);
      padding: calc(var(--spacing-unit) * 1.5) calc(var(--spacing-unit) * 2);
...
.tabs {
  ...
  flex-direction: column;
  width: 100%;
  /* Punkt odniesienia dla ukrytych inputow (position: absolute nizej).
     Bez tego pozycjonowalyby sie wzgledem okna - niewidoczne i tak, ale
     lepiej trzymac je w obrebie komponentu. */
  position: relative;
}
...
/* UKRYCIE ZACHOWUJACE FOKUSOWALNOSC.
   Wczesniej bylo tu display:none, ktore wyrzuca pole z kolejnosci tabulacji
   I z drzewa dostepnosci - zakladek nie dalo sie wtedy przelaczyc klawiatura
   ani odczytac czytnikiem ekranu. Technika clip (ta sama co w .sr-only)
   ukrywa wizualnie, ale zostawia element dostepnym: grupa radio odzyskuje
   natywna obsluge strzalkami. */
.tab-input {
  position: absolute;
  width: 1px;
  height: 1px;
...
@for $i from 1 through 10 {
  ...
  /* Obwodka focusa musi trafic na widoczna etykiete - samo radio jest
     przyciete do zera, wiec jego wlasny outline bylby niewidoczny. */
  .tab-input:nth-of-type(#{$i}):focus-visible ~ .tabs-header .tab-label:nth-of-type(#{$i}) {
    outline: var(--focus-ring-width) solid var(--focus-ring-color);
    outline-offset: -2px;
...
@layer layout {
  .admin-layout {
    ...
    // Zapobiega FOUC: te same reguly co wyzej, ale zaczepione o klase na
    // <html> - ja ustawia synchroniczny inline-skrypt w <head> (patrz
    // partials/head.html), zanim przegladarka zdazy cokolwiek narysowac.
    // .admin-layout wciaz dostaje swoja WLASNA klase pozniej, w
    // molique-script.js (do obslugi kliku) - ta regula tylko zapewnia,
    // ze wlasciwa szerokosc jest gotowa juz na pierwszej klatce, zamiast
    // "skakac" z pelnej (lg) po DOMContentLoaded.
    :root.sidebar-md & { --current-sidebar-width: var(--sidebar-width-md); }
    :root.sidebar-sm & { --current-sidebar-width: var(--sidebar-width-sm); }
...
@layer reset {
  ...
  @include meta.load-css("fonts"); // @font-face: Poppins
}
...
@layer utilities {
  ...
  /* API komponentu na zmiennych, nie na klasach: kolor i czas to wartosci
     CIAGLE i niesemantyczne, wiec kazda klasa byla by arbitralnym punktem na
     skali (inaczej niz .stock-bar-success, gdzie nazwa niesie stan).
     Deklarujemy je na kontenerze, zeby dalo sie nadpisac jednym
     style="--blob-1: #7c3aed" bez siegania do pseudoelementow. */
  .bg-blobs {
    --blob-1: rgba(var(--primary-rgb), 0.5);
  ...
  .bg-blobs::before, .bg-blobs::after {
    ...
    /* Rozmycie 50px, nie 80px. Przy 80px kazde rozmycie punktu bylo WIEKSZE
       niz nieregularnosc obrysu (~60px na plamie 600px), wiec organiczny
       ksztalt byl zamazany do kola i obrot pozostawal niewidoczny. */
    filter: blur(50px); z-index: -1;
    /* Krycie przez zmienna, zeby klatki mogly nim pulsowac, a warianty
       (np. .bg-blobs-deep) nadal ustawialy wlasny poziom bazowy. */
    --blob-opacity: 0.4;
    opacity: var(--blob-opacity);
    will-change: transform, opacity; transform: translate3d(0, 0, 0);
    animation: blobFloat var(--blob-speed) infinite alternate cubic-bezier(0.4, 0, 0.2, 1);
  }
  .bg-blobs::before { background-color: var(--blob-1); top: -10%; left: -10%; }
  .bg-blobs::after {
    background-color: var(--blob-2); bottom: -10%; right: -10%;
    /* Lustrzany obrys, żeby obie plamy nie były tym samym kształtem. */
    border-radius: 63% 37% 42% 58% / 62% 55% 45% 38%;
    /* OSOBNE klatki, nie te same z opoznieniem: wczesniej obie plamy jechaly
       rownolegle w te sama strone, co czytalo sie jako wspolne kolysanie
       zamiast niezaleznego dryfu. */
    animation-name: blobFloatAlt;
    /* 1.3x wolniej od pierwszej plamy. Mnoznik, a nie sztywna wartosc - dzieki
       temu zmiana --blob-speed przyspiesza obie, zachowujac rozjazd cykli. */
    animation-duration: calc(var(--blob-speed) * 1.3);
    animation-delay: calc(var(--blob-speed) * -0.6);
  }
  @keyframes blobFloat {
    0%   { transform: translate3d(0, 0, 0) scale(1) rotate(0deg);
           opacity: calc(var(--blob-opacity) * 0.7); }
    100% { transform: translate3d(12%, 18%, 0) scale(1.18) rotate(45deg);
           opacity: var(--blob-opacity); }
  }
  @keyframes blobFloatAlt {
    0%   { transform: translate3d(0, 0, 0) scale(1.12) rotate(0deg);
           opacity: var(--blob-opacity); }
    100% { transform: translate3d(-15%, -12%, 0) scale(0.92) rotate(-40deg);
  ...
  /* Wariant ZAWSZE CIEMNY - tak jak .navbar-pill, i z tego samego powodu:
     ma być pewnym podkładem pod biały tekst niezależnie od motywu strony.
     Wartość jest literalna, NIE var(--bg-body) ani var(--dark): te zmienne
     odwracają się w dark mode, więc w motywie jasnym tło zrobiłoby się jasne
     i biały tekst by zniknął. Podmiana przez style="--blobs-deep-bg: #123". */
  .bg-blobs-deep {
    --blobs-deep-bg: #0A0B14;
    background-color: var(--blobs-deep-bg);
    /* Mocniejsze krycie kolorow, bo na ciemnym tle rozmyte barwy gasna.
       Nadpisujemy --blob-1/--blob-2, wiec wlasny kolor podany w style="..."
       na tym samym elemencie i tak wygra (kolejnosc deklaracji inline). */
    --blob-1: rgba(var(--primary-rgb), 0.75);
    --blob-2: rgba(var(--info-rgb), 0.65);
    /* Poziom BAZOWY krycia - klatki pulsują względem niego. */
    &::before, &::after { --blob-opacity: 0.6; }
  }
...
@layer utilities {
  ...
  .bg-dark { background-color: #14162B !important; color: #F9F9F9 !important; }
  ...
  .bg-gradient-corners {
    ...
    &::before {
      ...
      background:
        radial-gradient(circle at 0% 0%, rgba(var(--primary-rgb), 0.22) 0%, transparent 45%),
        radial-gradient(circle at 100% 100%, rgba(var(--info-rgb), 0.18) 0%, transparent 45%);
      pointer-events: none;
    }
  }

  /* Ta sama łuna co wyżej, ale na jasnym tle - w przeciwienstwie do
     .bg-gradient-corners TO tlo odwraca sie normalnie z motywem (var(--bg-body)),
     bo tu nie ma zalozenia o bialym tekscie na wierzchu. */
  .bg-gradient-corners-light {
    position: relative;
    background-color: var(--bg-body);
    z-index: 1;
    overflow: hidden;
    ...
    &::before {
      ...
      background:
        radial-gradient(circle at 0% 0%, rgba(var(--primary-rgb), 0.09) 0%, transparent 45%),
        radial-gradient(circle at 100% 100%, rgba(var(--info-rgb), 0.07) 0%, transparent 45%);
      pointer-events: none;
    }
  }
...
.text-knockout-dark {
  ...
  /* =========================================
     9. FULL-BLEED / BREAKOUT (Mobile)
     ========================================= */
  /* Wyrywa element z paddingu .container na mobile, az do krawedzi ekranu -
     typowo karuzele, galerie zdjec, zdjecia hero wewnatrz tresci artykulu.
     Dziala TYLKO ponizej md (768px): .container ma tam padding-inline
     (patrz _base.scss), a na md+ jest juz z zamierzenia wezszy niz viewport,
     wiec breakout tam tylko psulby uklad.

     Technika "viewport centering" (width: 100vw + margin-left: 50% +
     translateX(-50%)) dziala niezaleznie od realnej wartosci paddingu
     rodzica - nie trzeba go znac ani przeliczac, dziala nawet gdy ktos
     nadpisze padding .container klasa narzedziowa. html ma juz globalnie
     overflow-x: clip (patrz _base.scss) - dzieki temu 100vw nie rozsadza
     ukladu poziomym scrollem, nawet gdy strona ma widoczny pasek przewijania
     (bez tej tarczy 100vw liczy sie WRAZ z szerokoscia scrollbara). */
  @include mq(sm, max) {
    .breakout-mobile {
      width: 100vw;
      margin-left: 50%;
...
/* Mikrocopy - CELOWO mniejsze niż tokeny --text-sm/--text-xs (13/12px),
   ...
   clamp() zamiast sztywnego rem - lekko rosną na dużych ekranach (o ok.
   1px), żeby nie stały się nieczytelne przy ekstremalnej gęstości
   pikseli, ale zostają wyraźnie mniejsze niż .text-1 nawet przy max. */
.text-xs { font-size: clamp(0.625rem, 0.6rem + 0.08vw, 0.6875rem) !important; }  /* 10px -> 11px */
.text-sm { font-size: clamp(0.6875rem, 0.66rem + 0.08vw, 0.75rem) !important; }  /* 11px -> 12px */
...
@layer base {
  ...
  *, *::before, *::after {
  ...
  /* Reset domyślnego stylu przeglądarki. Bez tego UA-owy
     "border-style: inset" + "margin-inline: auto" rozjeżdża się jako
     BEZPOŚREDNIE dziecko flexa (np. .card-body, każdy .d-flex.flex-column):
     auto-marginesy w osi poprzecznej flexboksa mają pierwszeństwo przed
     align-items:stretch i "zjadają" całą wolną przestrzeń zamiast
     rozciągnąć element, więc linia kurczy się do prawie zerowej szerokości
     (widać ją jako kropkę). Ten sam problem już raz naprawiono lokalnie w
     .modal-divider (_modal-context.scss) - to jest ta sama poprawka
     zgeneralizowana na wszystkie <hr>. */
  hr {
    width: 100%;
    flex-shrink: 0;
...
@layer components {
  ...
  /* =========================================
     ...
     ========================================= */
  /* Dzieci celuja w $btn-base-selectors (ta sama lista co baza wyzej), nie w
     literalna klase .btn - odkad kolor implikuje .btn (patrz komentarz przy
     IMPLIKACJA .btn), sam .btn-primary w grupie nie dostawal tu wczesniej
     zadnego z tych zaokraglen/nachodzenia obwodek. */
  .btn-group {
    display: inline-flex;
    align-items: stretch;
  ...
  /* 1. GLOW (Neonowa poświata)
     Cień hover NIE transitionuje bezposrednio (reflow) - siedzi na ::after,
     ktory animuje tylko opacity. Ten sam wzorzec co .hover-gpu-shadow w
     utilities/_animations.scss. Bazowy (spoczynkowy) cień zostaje na
     przycisku, bo jest staly - nigdy sie nie zmienia, wiec nie kosztuje. */
  .btn-glow {
    position: relative;
    isolation: isolate;
  ...
  /* 3. 3D PUSH (Fizyczny przycisk)
     Zamiast animowac Y-offset plaskiego box-shadow (reflow), "podstawa"
     siedzi na statycznym ::before (solid tlo, offset o 6px pod przyciskiem).
     Przycisk sam animuje wylacznie transform: translateY() - w miare
     wciskania "zblizy sie" do statycznej podstawy, dajac identyczny efekt
     znikajacego cienia bez animowania box-shadow ani razu. */
  .btn-3d {
  ...
  /* 5. GRADIENT ANIMATED (Płynący gradient)
     Zamiast animowac background-position (reflow/paint na kazdej klatce),
     gradient siedzi na ::before dwukrotnie szerszym od przycisku,
     przesuwanym przez transform: translateX() - identyczny efekt plynacego
     gradientu, zero animowanych wlasciwosci poza transform/opacity. Cien
     na hover tak samo jak w .btn-glow: osobny ::after, animowana tylko
     opacity. Tresc przycisku (z-index:auto) zostaje ponad obiema
     warstwami (z-index ujemny), bo isolation:isolate tworzy dla nich
     wlasny kontekst uwarstwienia. */
  .btn-gradient {
    position: relative;
    isolation: isolate;
    overflow: hidden;
...
@use 'components/lang-suggest' as *;
```
 ...
 * FontBody = FontHeading = Poppins (jeden krój, ciaśniejszy tracking - patrz
 * --font-family-base w _root.scss). Dwie nazwy zostają jako alias, nie jako
 * dwa różne kroje: gdyby projekt kiedyś wrócił do osobnego fontu dla body,
 * wystarczy zmienić --font-family-base, bez ruszania tego pliku.
 * Ścieżki względne do skompilowanego CSS w css/ (pliki w css/../fonts/).
 * font-display: swap - tekst widoczny od razu, bez FOIT.
 *
 * Kazda waga ma WLASNY plik latin-ext (polskie/czeskie/etc. znaki) pobrany
 * bezposrednio z Google Fonts, zamiast pozyczac glify z najblizszej wagi,
 * ktora akurat go miala - to poprzednie rozwiazanie dawalo widoczny rozjazd
 * grubosci kresek pomiedzy podstawowa lacina i znakami typu ą/ę/ł w tym
 * samym slowie przy wagach 100/200/300/500/600/800/900.
 */
...
@each $weight in $poppins-weights {
  @font-face {
    font-family: 'FontHeading';
    font-style: normal;
    font-weight: $weight;
    font-display: swap;
    src: url('../fonts/poppins-v24-latin-#{$weight}.woff2') format('woff2');
    unicode-range: $latin;
  }
  ...
  @font-face {
    ...
    font-weight: $weight;
    font-display: swap;
    src: url('../fonts/poppins-v24-latin-ext-#{$weight}.woff2') format('woff2');
...
:root {
  ...
  /* Fonty z systemowym fallbackiem. Baza i nagłówki obie na FontHeading
     (Poppins) - v2 design refresh (patrz artefakt "Modernizacja Molique
     Design System"). Jeden krój w calym systemie - FontBody zostal
     usuniety z _fonts.scss razem z Inter. */
  --font-family-base: 'FontHeading', system-ui, -apple-system, sans-serif;
  --font-family-heading: 'FontHeading', system-ui, -apple-system, sans-serif;

  /* PALETA v2 - Indygo jako brand, chlodniejszy neutral (Modernizacja 2026) */
  --primary: #5145E5;
  --primary-hover: #3D31CE;
  --light: #F9F9F9;
  --dark: #14162B;

  --secondary: #696D8F;
  --secondary-hover: #4E5273;
  --success: #12B76A;
  --success-hover: #039855;
  --danger: #F04438;
  --danger-hover: #D92D20;
  --warning: #F79009;
  --warning-hover: #DC6803;
  --info: #2E90FA;
  --info-hover: #1570EF;

  /* Kolory tła i tekstu */
  --bg-body: #F5F6FC; /* Chlodny neutral pod panele */
  ...
  /* ZMIENNE SIDEBARA ADMINA.
     ...
     znikają na ciemnym tle sidebara. sidebar-bg jest teraz polprzezroczysty
     (rgba, "szklany" v2 - patrz artefakt), stad zalezy od --sidebar-rgb
     ponizej zamiast literalnego hexa. */
  --sidebar-bg: rgba(var(--sidebar-rgb), 0.92);
  ...
  --primary-rgb: 81, 69, 229;     /* #5145E5 */
  --secondary-rgb: 105, 109, 143; /* #696D8F */
  --success-rgb: 18, 183, 106;    /* #12B76A */
  --danger-rgb: 240, 68, 56;      /* #F04438 */
  --warning-rgb: 247, 144, 9;     /* #F79009 */
  --info-rgb: 46, 144, 250;       /* #2E90FA */

  --dark-rgb: 20, 22, 43;         /* #14162B */
  --light-rgb: 249, 249, 249;     /* #F9F9F9 */
  --body-rgb: 245, 246, 252;      /* #F5F6FC */
  --bg-surface-rgb: 255, 255, 255; /* #FFFFFF - kanały --bg-surface, używane przez .bg-glass */
  --sidebar-rgb: 20, 22, 43;      /* = --dark-rgb - sidebar "szklany" v2 nad tlem strony */

  --btn-text-light: #FFFFFF;
  --btn-text-dark: var(--dark);
  --card-bg-subtle: rgba(var(--dark-rgb), 0.035);
  ...
  --transition-speed: 0.22s;      /* Baza */
  --transition-speed-fast: 0.15s;
  --transition-speed-slow: 0.3s;
  --transition-speed-slower: 0.4s;
  --transition-speed-chart: 0.8s; /* Wykresy - dluzsza animacja "rosniecia" */
  ...
  /* Cienie i Poświaty - v2: wielowarstwowe, barwione --dark-rgb zamiast
     czystej czerni (spojne z chlodniejszym neutralem calej palety) */
  --shadow-sm: 0 1px 2px rgba(var(--dark-rgb), 0.05), 0 4px 10px -2px rgba(var(--dark-rgb), 0.05);
  --shadow-md: 0 2px 4px rgba(var(--dark-rgb), 0.04), 0 14px 30px -8px rgba(var(--dark-rgb), 0.12);
  --shadow-lg: 0 40px 80px -32px rgba(var(--dark-rgb), 0.28), 0 12px 24px -8px rgba(var(--dark-rgb), 0.08);

  /* Nakladki pelnoekranowe (lightbox, offcanvas, popover) - zawsze czarne,
     niezaleznie od motywu (jak cienie wyzej). Kazdy komponent zachowuje
     WLASNA, dobrana dla siebie przezroczystosc (rgba(var(--backdrop-rgb), X))
     - ten token daje wspolne ZRODLO KOLORU do przyszlej zmiany (np. lekki
     odcien zamiast czystej czerni), nie wspolna przezroczystosc. */
  --backdrop-rgb: 0, 0, 0;
...
[data-theme="dark"] {
  ...
  /* 1. Kolory strukturalne (v2 - blisko czerni, ciepla/fioletowa, nie
     czysty czarny ani stary granat) */
  --bg-body: #0A0B14;
  --bg-surface: #141629; /* Nieco jaśniejszy od body (tworzy głębię kart) */
  --text-main: #ECEDF8;
  --text-muted: #9BA0C4;
  --border-color: #262A48;

  /* 2. Kolory semantyczne v2 (rozjasnione warianty indygo/success/danger itd. */
  --primary: #8B84FF;
  --primary-hover: #A5A0FF;

  --secondary: #8E93B5;
  --secondary-hover: #A9AECB;

  --success: #32D583;
  --success-hover: #6CE9A6;

  --danger: #FF7A72;
  --danger-hover: #FFA39D;

  --warning: #FDB022;
  --warning-hover: #FEC84B;

  --info: #63A8FF;
  --info-hover: #8EC2FF;

  /* Zmienne semantyczne odwrócone - light w dark mode = bg-surface (ta sama
     zaleznosc co przed v2, tylko z nowymi wartosciami) */
  --light: #141629;
  --dark: #ECEDF8;

  /* 3. Wartości RGB dla Dark Mode (Kluczowe dla klas -subtle i focus ringów!) */
  --primary-rgb: 139, 132, 255;
  --secondary-rgb: 142, 147, 181;
  --success-rgb: 50, 213, 131;
  --danger-rgb: 255, 122, 114;
  --warning-rgb: 253, 176, 34;
  --info-rgb: 99, 168, 255;

  --dark-rgb: 236, 237, 248;
  --light-rgb: 20, 22, 41;
  --body-rgb: 10, 11, 20;
  --bg-surface-rgb: 20, 22, 41;
  /* Sidebar NIE zmienia sie w dark mode (patrz UWAGA przy --sidebar-bg w
     :root) - to samo --sidebar-rgb co w light, zeby rgba(var(--sidebar-rgb))
     dalo dokladnie ten sam kolor niezaleznie od motywu strony. */
  --sidebar-rgb: 20, 22, 43;

  --btn-text-light: #0A0B14;

  /* Cienie v2 w dark mode: czysta czern (nie barwione --dark-rgb jak w
     light) - na ciemnym tle barwiony cien jest niewidoczny/bez sensu. */
  --shadow-sm: 0 1px 2px rgba(0, 0, 0, 0.5), 0 4px 12px -2px rgba(0, 0, 0, 0.4);
  --shadow-md: 0 2px 6px rgba(0, 0, 0, 0.5), 0 16px 34px -10px rgba(0, 0, 0, 0.6);
  --shadow-lg: 0 44px 90px -34px rgba(0, 0, 0, 0.85), 0 14px 28px -10px rgba(0, 0, 0, 0.5);

  --card-bg-subtle: rgba(255, 255, 255, 0.05);

  /* Ciemniej niż --bg-surface (#141629) i niż --bg-body (#0A0B14),
...
/* =========================================
   GALERIA PRODUKTU (Zdjecie glowne + miniatury)
   =========================================
   Struktura odtworzona z molique-shop.js, gdzie logika JS istniala od dawna
   (klik w miniature podmienia glowne zdjecie), ale komponent nigdy nie
   dostal CSS ani markupu wzorcowego - byl martwym kodem.

   .gallery-thumb to <button>, nie <a>/<div>: dziala z klawiatury (Tab, Enter)
   bez dopisywania czegokolwiek, tak jak reszta interaktywnych elementow
   frameworka. Stan aktywny/przygaszony (.is-active / .opacity-50) nadaje
   JS - .opacity-50 to gotowa klasa narzedziowa, nie nowa. */
.product-gallery {
  display: flex;
  flex-direction: column;
  gap: calc(var(--spacing-unit) * 1.5);
...
/* Warianty umiejscowienia miniatur. Domyslnie (bez modyfikatora) sa rzedem
   POD zdjeciem - dziala wszedzie, w tym na mobile.

   .product-gallery-left / -right ustawiaja je kolumna OBOK zdjecia, ale
   TYLKO od `md` w gore - kolumna miniatur obok duzego zdjecia nie miesci
   sie sensownie na waskim ekranie, wiec ponizej progu obie odmiany
   wracaja do domyslnego ukladu (rzad pod spodem), zgodnie z reszta
   frameworka (mobile = jedna kolumna).

   Kolejnosc w DOM zostaje ZAWSZE taka sama - najpierw .product-gallery-main,
   potem .product-gallery-thumbs. .product-gallery-left przestawia tylko
   kolejnosc WIZUALNA przez `order`, zeby markup nie musial sie zmieniac
   miedzy wariantami (ten sam powod co --tab-count czy inne modyfikatory
   ustawiane bez przepisywania HTML). */
...
.product-gallery-right {
  @include mq(md) {
    ...
    .product-gallery-thumbs {
      flex-direction: column;
      /* Bez tego dziedziczy flex-wrap: wrap z reguly bazowej. W ukladzie
         kolumnowym przegladarka potrafi otworzyc "druga kolumne" miniatur
         (cross-axis wrap), ktora renderuje sie poza widocznym pasem -
         ostatnia miniatura wyglada wtedy jakby znikla, mimo poprawnego
         adresu obrazka. */
      flex-wrap: nowrap;
    }
    ...
    .product-gallery-main {
      flex: 1;
      /* Bez tego flex-item nie kurczy sie ponizej szerokosci swojej
         tresci (domyslne min-width: auto) i potrafi rozepchac uklad. */
      min-width: 0;
    }
  }
}
...
.gallery-thumb {
  /* Reset stylow przegladarki dla <button> */
  padding: 0;
  background: none;
  cursor: pointer;
